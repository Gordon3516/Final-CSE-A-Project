...
class Game(arcade.Window):
    ...
    def on_draw(self):
        ...
        if ((len(self.wall_hit_list) > 0 or (len(self.self_hit_list) > 0 and self.invincible_bool == 0)) and self.glob > 100):
            for i in range(1):
                if not self.death_sound_player or not self.death_sound_player.playing:
                    self.death_sound_player = arcade.play_sound(self.death_sound)
            self.snake_dx = 0
            self.snake_dy = 0
